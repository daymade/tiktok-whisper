...
#### On macOS (example-host.local)

```bash
# SSH into the server
ssh user@example-host.local
...
providers:
  whisper_server_lan:
    ...
    enabled: true
    settings:
      base_url: "http://example-host.local:8080"
      # Or use IP directly
      # base_url: "http://192.0.2.10:8080"
      language: "zh"
      response_format: "verbose_json"
...
1. **Connection Timeout**
   ...
   ssh user@example-host.local "pgrep -f whisper-server"
...
2. **503 Service Unavailable**
   ...
   - Check server logs:
     ```bash
     ssh user@example-host.local "tail -f whisper-server.log"
     ```

3. **Proxy Issues**
   ```bash
   # Bypass proxy for LAN
   export NO_PROXY="192.0.2.10,example-host.local"
   
   # Or use SSH tunnel
   ssh -L 8080:localhost:8080 user@example-host.local -N
   ```
...
# Test inference
curl -X POST http://192.0.2.10:8080/inference \
    -F "file=@audio.wav" \
    -F "response_format=json"

# View server logs
ssh user@example-host.local "tail -f /path/to/whisper-server.log"
...
# Monitor performance
curl -s http://192.0.2.10:8080/metrics | grep whisper_
```
